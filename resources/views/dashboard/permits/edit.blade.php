    ...
    <div class="container-xxl flex-grow-1 container-p-y">
        <div class="card mb-6">
            ...
            <div class="card-body pt-4">
                <form method="POST" action="{{ route('dashboard.permits.update', $permit->id) }}" enctype="multipart/form-data">
                    @csrf
                    @method('PUT')

                    <div class="row p-5">
                        <h3>{{ __('Edit Permit') }}</h3>
                        <div class="mb-4 col-md-12">
                            ...
                            <select class="form-control select2 @error('type') is-invalid @enderror" id="type"
                                ...
                                <option value="karachi" {{ old('type', $permit->type) == 'karachi' ? 'selected' : '' }}>
                                    {{ __('Karachi') }}
                                </option>
                                <option value="lasbella" {{ old('type', $permit->type) == 'lasbella' ? 'selected' : '' }}>
                                    {{ __('Lasbella') }}
                                </option>
                                <option value="peshawar" {{ old('type', $permit->type) == 'peshawar' ? 'selected' : '' }}>
                                    {{ __('Peshawar') }}
                                </option>
                                <option value="gilgit" {{ old('type', $permit->type) == 'gilgit' ? 'selected' : '' }}>
                                    {{ __('Gilgit') }}
                                </option>
                                <option value="other" {{ old('type', $permit->type) == 'other' ? 'selected' : '' }}>
                                    {{ __('Other') }}
                                </option>
                            </select>
                            @error('type')
                            ...
                            <select class="form-control select2 @error('vehicle_id') is-invalid @enderror" id="vehicle_id"
                                ...
                                @foreach ($vehicles as $vehicle)
                                    <option value="{{ $vehicle->id }}"
                                        {{ old('vehicle_id', $permit->vehicle_id) == $vehicle->id ? 'selected' : '' }}>
                        ...
                        <div class="mb-4 col-md-12">
                            ...
                            <input class="form-control @error('permit_date') is-invalid @enderror" type="date"
                                ...
                                placeholder="{{ __('Enter permit date') }}" value="{{ old('permit_date', $permit->permit_date) }}" />
                            @error('permit_date')
                                <span class="invalid-feedback" role="alert">
                                    <strong>{{ $message }}</strong>
                                </span>
                        ...
                        <div class="mb-4 col-md-12">
                            ...
                            <textarea class="form-control @error('permit_details') is-invalid @enderror" id="permit_details" name="permit_details"
                                placeholder="{{ __('Enter permit details...') }}">{{ old('permit_details', $permit->permit_details) }}</textarea>
                        ...
                        <div class="mb-4 col-md-12">
                            ...
                            <textarea class="form-control @error('notes') is-invalid @enderror" id="notes" name="notes"
                                placeholder="{{ __('Enter notes...') }}">{{ old('notes', $permit->notes) }}</textarea>
                            @error('notes')
                                <span class="invalid-feedback" role="alert">
                                    <strong>{{ $message }}</strong>
                                </span>
                        ...
                        <div id="from_user_select" class="mb-4 col-md-12">
                            <select class="form-control select2" name="permit_holder_id">
                                ...
                                @foreach ($users as $user)
                                    <option value="{{ $user->id }}" {{ old('permit_holder_id', $permit->permit_holder_id) == $user->id ? 'selected' : '' }}>{{ $user->name }}</option>
                                @endforeach
                            </select>
                        </div>
