        ...
        <form id="formAccountSettings" method="POST" action="{{ route('profile.update', $profile->id) }}"
            ...
            <div class="row">
                        ...
                        @if (isset($countries) && count($countries) > 0)
                            @foreach ($countries as $country)
                                <option value="{{ $country->name }}"
                                    {{ $country->name == $profile->country ? 'selected' : '' }}>{{ $country->name }}
                                </option>
                            @endforeach
                        @endif
                    </select>
                        ...
                        @if (isset($languages) && count($languages) > 0)
                            @foreach ($languages as $language)
                                <option value="{{ $language->name }}"
                                    {{ $language->name == $profile->language ? 'selected' : '' }}>
                ...
                <div class="mb-4 col-md-4">
                    ...
                    <select id="marital_status" name="marital_status" class="select2 form-select @error('marital_status') is-invalid @enderror">
                        ...
                        @if (isset($maritalStatuses) && count($maritalStatuses) > 0)
                            @foreach ($maritalStatuses as $maritalStatus)
                                <option value="{{ $maritalStatus->name }}"
                                    {{ $maritalStatus->name == $profile->marital_status ? 'selected' : '' }}>
